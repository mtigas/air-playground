...
function SHA256_string_to_array(str) {
  var len = str.length;
...
function SHA256_array_to_hex_string(ary) {
  var res = "";
...
function SHA256_array_to_raw_string(ary) {
	var res = "";
	for(var i = 0; i < ary.length; i++)
...
function SHA256_write(msg) {
  if (typeof(msg) == "string")
    SHA256_buf = SHA256_buf.concat(SHA256_string_to_array(msg));
  else
...
function SHA256_hash(msg) {
  ...
  SHA256_write(msg);
  res = SHA256_finalize();
  return SHA256_array_to_hex_string(res);
}
...
function HMAC_SHA256_init(key) {
  if (typeof(key) == "string")
    HMAC_SHA256_key = SHA256_string_to_array(key);
  else
